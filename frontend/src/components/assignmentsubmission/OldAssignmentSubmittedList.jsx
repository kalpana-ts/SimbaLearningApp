...
export default function AssignmentSubmittedList({assignment,user}) {
    ...
    const assignmentsSubmittedByUser = assignmentsSubmitted.filter((p) => p.user.id === user.id);
    console.log("assignmentsSubmittedByUser",assignmentsSubmittedByUser);
    const assignementSubmittedByUser = assignmentsSubmittedByUser.map(assignsubmit =><AssignmentSubmittedCard key={assignsubmit.id} assignmentSubmitted={assignsubmit}/>);
